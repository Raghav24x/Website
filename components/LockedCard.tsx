...
export default function LockedCard({ resource }: { resource: Resource }) {
  ...
  const catClass = categoryColors[resource.category] ?? "text-[#FFF9F5]/50 bg-[rgba(255,255,255,0.05)] border-[rgba(255,255,255,0.1)]";
  const ftClass = resource.fileType ? (fileTypeBg[resource.fileType] ?? "bg-[rgba(255,255,255,0.1)] text-[#FFF9F5]/70") : "";

  return (
    <div className={`relative rounded-2xl border overflow-hidden flex flex-col transition-all duration-200 ${
      isPaid
      ...
      {/* Badge */}
      {resource.badge && (
        <div className="absolute top-3 right-3 px-2 py-0.5 rounded-full bg-[#FF8C42]/20 border border-[rgba(255,140,66,0.4)] text-[#FF8C42] text-[10px] font-bold uppercase tracking-wide">
          {resource.badge}
        </div>
      )}
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        ...
        {/* Title */}
        <h3 className={`font-semibold text-sm leading-snug ${isPaid ? "text-[#FFF9F5]/50" : "text-[#FFF9F5]"}`}>
          {resource.title}
        </h3>

        {/* Description — blurred for paid */}
        <div className="relative">
          <p className={`text-xs leading-relaxed ${isPaid ? "text-[#FFF9F5]/40 blur-[3px] select-none" : "text-[#FFF9F5]/60"}`}>
